...
export default function HomePage() {
  ...
  const [tournamentName, setTournamentName] = useState('Turniej Sportowy');
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (settingsData) {
        setStatus(settingsData.status?.phase || 'registration');
        const name = settingsData.settings?.find(s => s.key === 'tournament_name')?.value;
        if (name) setTournamentName(name);
      }
      setLoading(false);
    });

    // Also fetch public status
  ...
  }[status] || { label: status, color: 'badge-gray' };
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <p style={{ color: 'var(--text2)', marginTop: '0.25rem' }}>Panel publiczny — podgląd wyników</p>
      </div>

      <div className="status-bar">
        <div className={`status-dot ${status === 'active' ? '' : 'inactive'}`} />
        <div>
        ...
        {status === 'registration' && !user && (
          <span style={{ color: 'var(--text2)', fontSize: '0.875rem' }}>
            🔑 Zaloguj się przez Discord, aby zarejestrować drużynę
          </span>
        )}
      </div>
      ...
      <div className="grid-2" style={{ marginBottom: '2rem' }}>
        {/* Active matches */}
        <div className="card">
          <div className="section-title">⚔️ Aktualne mecze</div>
          {activeMatches.length === 0 ? (
            <div style={{ color: 'var(--text2)', padding: '1rem 0', textAlign: 'center' }}>
              {status === 'registration' ? 'Turniej jeszcze nie wystartował' : 'Brak aktywnych meczów'}
            </div>
          ) : (
      ...
      {standings.length > 0 && (
        <div className="card">
          <div className="section-title">🏅 Wyniki drużyn</div>
